...
async def validar(doc: str = "", request: Request = None):
    try:
        ...
        <body><div class="page">
            <div class="header">
                <div style="display:flex;align-items:center;gap:14px"><img src="{LOGO_URL}"><div><div style="font-weight:700;font-size:16px">{NOME_CLINICA}</div><div style="font-size:11px;opacity:.85;margin-top:3px">{ENDERECO}</div></div></div>
                <div style="text-align:right"><span style="background:white;color:#0f3554;padding:6px 14px;border-radius:20px;font-size:12px;font-weight:700">DOCUMENTO OFICIAL</span></div>
            </div>
            <div class="content">
                ...
                <div class="grid">
                    ...
                    <div class="field full"><span class="l">Observações / Finalidade</span><span class="v">{info.get('obs','Atestado para fins de comprovação.')}</span></div>
                </div>
                ...
                <div class="grid">
                    ...
                    <div class="field"><span class="l">Validade</span><span class="v">Indeterminada</span></div>
            ...
            <div class="footer">
                {NOME_CLINICA} • {ENDERECO} • Documento gerado eletronicamente - Lei 14.063/2020
            </div>
        </div></body></html>
        """
...
async def admin_page(request: Request):
    if not check_auth(request):
        ...
        <form method="post" action="/login" style="background:white;padding:35px;border-radius:14px;width:320px"><h3 style="text-align:center;color:#0f3554">PAINEL PRO</h3>
        ...
        <button style="width:100%;padding:12px;background:#0f3554;color:white;border:none;border-radius:8px;margin-top:10px;font-weight:700">ENTRAR</button></form></body></html>"""
    docs = carregar()
    linhas = ""
    for cod, d in reversed(list(docs.items())):
        link = f"https://validacao-atesdado-medico.onrender.com/validar?doc={cod}"
        linhas += f"<tr><td><b>{cod}</b></td><td>{d.get('paciente','')}</td><td>{d.get('data','')}</td><td><a href='{link}' target='_blank'>Ver</a></td></tr>"
    ...
    <div style="display:flex;justify-content:space-between"><h2>Gerador PRO - {NOME_CLINICA}</h2><a href="/logout" style="color:#c00">Sair</a></div>
    <form method="post" action="/admin-salvar">
    <div style="display:grid;grid-template-columns:2fr 1fr 1fr;gap:10px">
        <input name="paciente" placeholder="Nome *" required style="padding:10px;border:1px solid #ddd;border-radius:6px">
        ...
        <input name="nascimento" placeholder="Nascimento" style="padding:10px;border:1px solid #ddd;border-radius:6px">
        <input name="idade" placeholder="Idade" style="padding:10px;border:1px solid #ddd;border-radius:6px">
        <input name="sexo" placeholder="Sexo" style="padding:10px;border:1px solid #ddd;border-radius:6px">
        <input name="codigo" placeholder="Código * ex: JB-93495" required style="padding:10px;border:1px solid #ddd;border-radius:6px">
        <input name="data" placeholder="Data" style="padding:10px;border:1px solid #ddd;border-radius:6px">
        <input name="horario" placeholder="Horário" style="padding:10px;border:1px solid #ddd;border-radius:6px">
        <input name="cid" placeholder="CID" style="padding:10px;border:1px solid #ddd;border-radius:6px">
        <input name="cid_desc" placeholder="Desc CID" style="padding:10px;border:1px solid #ddd;border-radius:6px">
        <input name="dias" placeholder="Dias afast." style="padding:10px;border:1px solid #ddd;border-radius:6px">
        <input name="afastamento" placeholder="Período" style="padding:10px;border:1px solid #ddd;border-radius:6px">
        <input name="unidade" placeholder="Unidade" value="{NOME_CLINICA}" style="padding:10px;border:1px solid #ddd;border-radius:6px;grid-column: span 2">
        <input name="endereco" placeholder="Endereço do Paciente / Unidade" value="{ENDERECO}" style="padding:10px;border:1px solid #ddd;border-radius:6px;grid-column: span 2">
        <input name="medico" placeholder="Médico" style="padding:10px;border:1px solid #ddd;border-radius:6px">
        <input name="crm" placeholder="CRM" style="padding:10px;border:1px solid #ddd;border-radius:6px">
        ...
        <input name="obs" placeholder="Observações" style="padding:10px;border:1px solid #ddd;border-radius:6px;grid-column: span 3">
    </div>
    <button style="width:100%;padding:14px;background:#0f3554;color:white;border:none;border-radius:8px;margin-top:15px;font-weight:800">💾 SALVAR</button>
    </form><hr><table border="1" cellpadding="10" style="width:100%;border-collapse:collapse"><tr style="background:#0f3554;color:white"><th>Código</th><th>Paciente</th><th>Data</th><th>Link</th></tr>{linhas}</table></div></body></html>
    """
